...
const getOrders = async (req, res) => {
  try {
    const query = `SELECT *, (SELECT full_name FROM customers WHERE id = customer_id) AS customer_name,
                             ...
                             SELECT created(NOW() :: DATE, 'dd/mm/yyyy'))
...
const updateOrders = async (req, res) => {
  try {
    const query = `UPDATE orders SET pick_location = $1, drop_location = $2, pick_time = $3, drop_time = $4, updated = now() WHERE id = $5 RETURNING *`;
    const result = await pool.query(query, [
